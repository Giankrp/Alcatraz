...
export const useCrypto = () => {
  ...
  const importRawKey = async (rawKey: Uint8Array): Promise<CryptoKey> => {
    return window.crypto.subtle.importKey("raw", rawKey as any, ALGORITHM, false, [
      "encrypt",
      "decrypt",
    ])
  }
   ...
   */
  const deriveKEK = async (password: string, salt: Uint8Array): Promise<CryptoKey> => {
   ...
   */
  const encryptMasterKey = async (masterKeyBase64: string, password: string) => {
   ...
   */
  const hashMasterPassword = async (password: string, email: string): Promise<string> => {
    const enc = new TextEncoder()
