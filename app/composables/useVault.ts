...
export const useVault = () => {
  ...
  const fetchItems = async (): Promise<void> => {
    ...
    try {
      const [regularItems, trashedItems] = await Promise.all([
        $fetch<VaultItemResponse[]>(`${config.public.apiBase}/api/vault/items`, {
          credentials: "include",
        }),
        $fetch<VaultItemResponse[]>(`${config.public.apiBase}/api/vault/trash`, {
          credentials: "include",
        }),
      ])
  ...
  const addItem = async (item: Omit<VaultItem, "id" | "icon" | "trashed">): Promise<VaultItem> => {
    ...
    try {
      ...
      const cryptoResult = await encryptData(sensitiveData, masterKey.value)
  ...
  const updateItem = async (id: string, updatedFields: Partial<VaultItem>): Promise<void> => {
    ...
    try {
      ...
      const cryptoResult = await encryptData(sensitiveExtracted, masterKey.value)
  ...
  const deleteItemPermanent = async (id: string) => {
    await $fetch(`${config.public.apiBase}/api/vault/items/${id}/permanent`, {
      method: "DELETE",
      credentials: "include",
    })
