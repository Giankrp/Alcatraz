<script setup lang="ts">
  ...
  const { setMasterKey, twoFactorPendingPassword, clearTwoFactorPendingPassword } =
    useMasterPassword()
  ...
  >
    ...
    <UContainer>
      <div class="mx-auto max-w-sm relative z-10">
        <UCard
          class="glass-card-dark transition-all duration-500 hover:shadow-[0_0_50px_rgba(16,185,129,0.1)]"
          :ui="{ body: 'p-8 sm:p-10', header: 'p-0' }"
        >
          <div class="text-center mb-10">
            <div
              class="size-16 bg-emerald-500/10 border border-emerald-500/20 rounded-2xl flex items-center justify-center mx-auto mb-6 shadow-[0_0_20px_rgba(16,185,129,0.1)]"
            >
              <UIcon
                :name="isBackupCode ? 'i-heroicons-shield-exclamation' : 'i-heroicons-shield-check'"
          ...
          <form @submit.prevent="onVerify" class="space-y-8">
            <div class="space-y-2">
              <p
                class="text-[10px] uppercase tracking-[0.2em] font-bold text-zinc-600 mb-4 text-center"
              >
              ...
              <UInput
                ...
                :ui="{
                  base: 'input-tech h-16 text-center text-2xl font-bold rounded-xl border-zinc-800 bg-black/40',
                }"
                autofocus
              ...
              <p
                v-if="errorMsg"
                class="text-[10px] text-red-500 font-mono uppercase text-center mt-2 animate-pulse"
              >
                {{ errorMsg }}
              </p>
            </div>
          ...
          <template #footer>
            <div class="flex justify-center pt-2">
              <ULink
                to="/login"
                class="text-[10px] font-bold uppercase tracking-[0.3em] text-zinc-700 hover:text-white transition-colors"
              >
...
<style scoped>
  ...
  .login-bg {
    background:
      radial-gradient(circle at top right, rgba(16, 185, 129, 0.05), transparent 40%),
      radial-gradient(circle at bottom left, rgba(16, 185, 129, 0.03), transparent 40%), #09090b;
  }
