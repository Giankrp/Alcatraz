...
const { setMasterKey, userEmail, clearMasterKey, clearUserEmail } =
    useMasterPassword();
    ...
    meta: [
        {
            name: "description",
            content:
                "Introduce tu contraseña maestra para acceder a tu bóveda segura.",
        },
...
const email = computed(
    () => session.value?.user?.email ?? userEmail.value ?? "",
);
    ...
    async (newEmail) => {
        ...
        try {
            const res = await $fetch<{ exists: boolean }>(
                `${config.public.apiBase}/api/auth/exists`,
                {
...
const onSubmit = async () => {
    ...
    try {
        ...
        const authHash = await hashMasterPassword(
        ...
        // 3. Login
        const loginRes = await $fetch<any>(
        ...
        if (isRegisterMode.value) {
            error.value =
                err?.data?.message ||
                "Error al crear la cuenta. Puede que ya exista una cuenta con ese email.";
        } else {
            error.value =
                err?.data?.message ||
                "Credenciales incorrectas. Verifica tu contraseña maestra.";
        }
    } finally {
        loading.value = false;
    }
                ...
                >
                    ...
                    >
                        <UIcon
                            ...
                            class="w-8 h-8 text-white/60 animate-spin"
                        />
