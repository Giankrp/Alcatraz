<script setup lang="ts">
  ...
  const { setMasterKey, userEmail, clearMasterKey, clearUserEmail } = useMasterPassword()
  ...
  useHead({
    title: computed(() =>
      isRegisterMode.value ? t("unlock.titleCreate") : t("unlock.titleUnlock"),
    ),
    meta: [
      {
        name: "description",
        content: t("unlock.descUnlock"),
      },
      { property: "og:title", content: `${t("unlock.titleUnlock")} — Alcatraz` },
    ],
  })

  // Si no hay ninguna sesión activa (ni OAuth ni backend), redirigir a login
  watch(
    [() => status.value, hasBackendSession],
    () => {
  ...
  const onSubmit = async () => {
    if (!masterPassword.value || masterPassword.value.length < 8) {
      error.value = t("unlock.errors.passwordMin")
      return
    }

    if (isRegisterMode.value && masterPassword.value !== confirmPassword.value) {
      error.value = t("unlock.errors.passwordMismatch")
      return
    }
    ...
    try {
      // 1. Hash determinista para Auth
      const authHash = await hashMasterPassword(masterPassword.value, email.value)

      if (isRegisterMode.value) {
        // 2. Generar y cifrar Master Key para registro
        const newMasterKey = generateMasterKey()
        const protectedKeyData = await encryptMasterKey(newMasterKey, masterPassword.value)

        // 3. Generar Recovery Key y cifrar la MK con ella
        const rk = generateRecoveryKey()
        const recoveryProtectedKeyData = await encryptMasterKey(newMasterKey, rk)
      ...
      if (loginRes.require_2fa) {
        ...
        // 4. Descifrar la Master Key recibida
        const masterKey = await decryptMasterKey(
          loginRes.protected_master_key,
          masterPassword.value,
          loginRes.master_key_salt,
          loginRes.master_key_iv,
        )
      ...
      if (isRegisterMode.value) {
        error.value = err?.data?.message || t("unlock.errors.registerError")
      } else {
        error.value = err?.data?.message || t("unlock.errors.incorrectPassword")
      }
    } finally {
      loading.value = false
    }
...
<template>
  <div
    class="relative min-h-screen login-bg text-white overflow-hidden grid place-items-center py-16"
  >
        ...
        >
          ...
          <template v-else>
            ...
            <div class="text-center mb-6">
              ...
              >
                <UIcon
                  :name="isRegisterMode ? 'i-heroicons-shield-check' : 'i-heroicons-lock-closed'"
                  class="w-7 h-7 text-white"
                />
              </div>
              <h1 class="text-xl font-semibold">
                {{ isRegisterMode ? $t("unlock.titleCreate") : $t("unlock.titleUnlock") }}
              </h1>
              <p class="text-sm text-white/60 mt-1">
                {{ isRegisterMode ? $t("unlock.descCreate") : $t("unlock.descUnlock") }}
              </p>
            </div>
            ...
            <div class="mb-5">
              ...
              <div
                class="flex items-center gap-2 px-3 py-2.5 rounded-md bg-white/5 border border-white/10"
              >
                ...
                <UIcon
                  ...
                  class="w-4 h-4 text-green-400 shrink-0 ml-auto"
                />
              </div>
            </div>
            ...
            <form @submit.prevent="onSubmit" class="space-y-4">
              ...
              <p class="text-xs text-white/40">
                {{ isRegisterMode ? $t("unlock.helpTextCreate") : $t("unlock.helpTextUnlock") }}
              </p>
              ...
              <UButton
                ...
                :label="isRegisterMode ? $t('unlock.buttonCreate') : $t('unlock.buttonUnlock')"
                :icon="isRegisterMode ? 'i-heroicons-shield-check' : 'i-heroicons-lock-open'"
                color="neutral"
                variant="solid"
                block
                size="lg"
            ...
            <div class="mt-5 pt-4 border-t border-white/10 text-center">
              <p class="text-sm text-white/50">
                {{ isRegisterMode ? $t("unlock.hasAccount") : $t("unlock.otherAccount") }}
                <button
                  type="button"
                  @click="handleSignOut"
                  class="font-medium text-white/80 hover:text-white cursor-pointer ml-1 transition-colors"
                >
  ...
  <UModal v-model:open="showRecoveryModal" :dismissible="false">
    <template #content>
      <div
        class="p-8 bg-black border border-emerald-500/20 rounded-2xl max-w-md mx-auto shadow-[0_0_50px_rgba(0,0,0,1)] space-y-6"
      >
        <div class="text-center">
          <div
            class="mx-auto size-14 bg-emerald-500/10 border border-emerald-500/20 rounded-full flex items-center justify-center mb-4"
          >
            <UIcon name="i-heroicons-shield-check" class="size-7 text-emerald-500" />
          </div>
          <h3 class="text-lg font-bold text-zinc-100 uppercase tracking-widest">
            {{ $t("unlock.modalTitle") }}
          </h3>
          <p class="text-xs text-zinc-500 mt-2 leading-relaxed">
            {{ $t("unlock.modalDescPart1") }}
            <strong class="text-emerald-500">{{ $t("unlock.modalDescPart2") }}</strong
            >{{ $t("unlock.modalDescPart3") }}
          </p>
        </div>

        <div class="p-4 bg-zinc-950 border border-emerald-500/30 rounded-lg text-center">
          <code class="text-emerald-400 font-mono text-sm tracking-widest break-all select-all">{{
