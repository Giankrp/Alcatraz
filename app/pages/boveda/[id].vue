<script setup lang="ts">
  ...
  const handleDelete = async () => {
    ...
    try {
      if (isTrashed.value) {
        ...
        toast.add({
          title: t("vault.itemDetails.deleted"),
          description: t("vault.itemDetails.deletedDesc"),
          color: "success",
        ...
        toast.add({
          title: t("vault.itemDetails.trashed"),
          description: t("vault.itemDetails.trashedDesc"),
          color: "success",
          icon: "i-heroicons-trash",
        })
      }
      ...
      toast.add({
        title: t("vault.itemDetails.error"),
        description: t("vault.itemDetails.errorRequest"),
        color: "error",
  ...
  const handleRestore = async () => {
    try {
      ...
      toast.add({
        title: t("vault.itemDetails.restored"),
        description: t("vault.itemDetails.restoredDesc"),
        color: "success",
      ...
      toast.add({
        title: t("vault.itemDetails.error"),
        description: t("vault.itemDetails.errorRestore"),
        color: "error",
      })
    }
  }
  ...
  const loadItem = async () => {
    ...
    if (itemId.value) {
      try {
        ...
        toast.add({
          title: t("vault.itemDetails.error"),
          description: t("vault.itemDetails.errorDecrypt"),
          color: "error",
        })
      }
    }
  ...
  const copyToClipboard = (text: string, label: string) => {
    navigator.clipboard
      ...
      .then(() => {
        toast.add({
          title: t("vault.itemDetails.copied"),
          description: t("vault.itemDetails.copiedDesc", { field: label }),
          color: "success",
      ...
      .catch(() => {
        toast.add({
          title: t("vault.itemDetails.error"),
          description: t("vault.itemDetails.copyErrorDesc"),
          color: "error",
          icon: "i-heroicons-x-circle",
        })
      })
  ...
  const handleSave = (data: any) => {
    if (item.value) {
      ...
      toast.add({
        title: t("vault.itemDetails.updated"),
        description: t("vault.itemDetails.updatedDesc"),
        color: "success",
        icon: "i-heroicons-check-circle",
      })
    }
  ...
  const timeSince = (date?: string) => {
    if (!date) return t("vault.itemDetails.time.unknown")
    const seconds = Math.floor((new Date().getTime() - new Date(date).getTime()) / 1000)
    let interval = seconds / 31536000
    if (interval > 1) return t("vault.itemDetails.time.years", { count: Math.floor(interval) })
    interval = seconds / 2592000
    if (interval > 1) return t("vault.itemDetails.time.months", { count: Math.floor(interval) })
    interval = seconds / 86400
    if (interval > 1) return t("vault.itemDetails.time.days", { count: Math.floor(interval) })
    interval = seconds / 3600
    if (interval > 1) return t("vault.itemDetails.time.hours", { count: Math.floor(interval) })
    interval = seconds / 60
    if (interval > 1) return t("vault.itemDetails.time.minutes", { count: Math.floor(interval) })
    return t("vault.itemDetails.time.justNow")
  }
          ...
          <div class="w-full">
            ...
            <div v-else>
                  ...
                  <div class="p-10 flex items-start justify-between">
                    <div class="flex items-center gap-6">
                      ...
                      <div>
                        ...
                        <p class="text-xs text-zinc-500 uppercase tracking-widest mt-1 font-bold">
                          {{ folderName }}
                        </p>
                      </div>
                    </div>
                  ...
                  <div class="p-10 pt-0 space-y-4">
                    ...
                    >
                      <label
                        ...
                        >{{ $t("vault.itemDetails.username") }}</label
                      >
                      <div class="flex items-center justify-between">
                        <span class="text-zinc-200 text-xl font-light">{{
                          asPassword.username
                        }}</span>
                        <UButton
                          ...
                          variant="ghost"
                          @click="
                            copyToClipboard(asPassword.username!, t('vault.itemDetails.username'))
                          "
                        />
                      </div>
                    </div>
                    ...
                    >
                      <label
                        class="text-[9px] font-bold text-zinc-600 uppercase tracking-widest mb-1 block"
                        >{{ $t("vault.itemDetails.password") }}</label
                      >
                      <div class="flex items-center justify-between">
                        <span class="text-white font-mono text-2xl tracking-tighter">{{
                        ...
                        <div class="flex items-center gap-1">
                          ...
                          <UButton
                            ...
                            variant="ghost"
                            @click="
                              copyToClipboard(asPassword.password!, t('vault.itemDetails.password'))
                            "
                          />
                        </div>
                      </div>
              ...
              >
                ...
                >
                  <p class="whitespace-pre-wrap text-zinc-400 text-xl leading-relaxed">
                    {{ asNote.note }}
                  </p>
                  <UButton
                    icon="i-heroicons-document-duplicate"
                    ...
                    @click="copyToClipboard(asNote.note!, t('vault.itemDetails.notes'))"
                  />
                </div>
              </div>
              ...
              <div v-else class="w-full">
                ...
                >
                  ...
                  >
                    <div class="text-[10px] font-mono tracking-[0.5em] text-white/20">
                      ENCRYPTED CREDIT ASSET
                    </div>
                    <div class="text-3xl font-mono text-white tracking-widest text-center">
                      {{
                        showPassword ? asCard.number : "•••• •••• •••• " + asCard.number?.slice(-4)
                      }}
                    </div>
                    <div class="flex justify-between items-end">
                      <div class="uppercase text-xs font-light text-zinc-400 tracking-widest">
                        {{ asCard.holder }}
                      </div>
      ...
      >
        <div class="w-full lg:w-96 space-y-8">
          ...
          >
            ...
            ></div>
            <h3 class="text-[10px] font-bold text-zinc-500 uppercase tracking-[0.3em] mb-6">
              {{ $t("vault.itemDetails.securityIntegrity") }}
            </h3>
            ...
            <div class="space-y-6">
              ...
              <div v-if="passwordStrength !== null" class="space-y-4">
                <div class="space-y-2">
                  <div class="flex justify-between items-end">
                    <span class="text-xs text-zinc-400 font-medium tracking-tight">{{
                      $t("vault.itemDetails.passwordStrength")
                    }}</span>
                    <span
                      class="text-xs font-bold"
                      :class="passwordStrength > 75 ? 'text-emerald-500' : 'text-amber-500'"
                      >{{
                        passwordStrength > 75
                          ? $t("vault.itemDetails.strengthRobust")
                          : $t("vault.itemDetails.strengthVulnerable")
                      }}</span
                    >
                  </div>
                  <div class="h-1.5 w-full bg-zinc-900 rounded-full overflow-hidden">
                    <div
                      ...
                      :class="(passwordStrength || 0) > 75 ? 'bg-emerald-500' : 'bg-amber-500'"
                    ></div>
                  </div>
                </div>
              </div>
              ...
              <div class="space-y-4 pt-4 border-t border-white/5">
                <div class="flex justify-between group/meta">
                  <span class="text-[10px] uppercase text-zinc-600 font-bold tracking-widest">{{
                    $t("vault.itemDetails.created")
                  }}</span>
                ...
                <div class="flex justify-between group/meta">
                  <span class="text-[10px] uppercase text-zinc-600 font-bold tracking-widest">{{
                    $t("vault.itemDetails.visibility")
                  }}</span>
                  <span class="text-xs text-emerald-500 font-bold tracking-widest">{{
                    $t("vault.itemDetails.privateNode")
                  }}</span>
                </div>
              </div>
            </div>
          ...
          >
            <h3 class="text-[10px] font-bold text-zinc-700 uppercase tracking-[0.3em] mb-4">
              {{ $t("vault.itemDetails.encryptionProps") }}
            </h3>
          ...
          <div class="pt-8 border-t border-white/5 space-y-3">
            <UButton
              ...
              variant="soft"
              block
              :label="$t('vault.itemDetails.restaurar')"
            ...
            <UButton
              ...
              :label="
                isTrashed
                  ? $t('vault.itemDetails.deletePermanent')
                  : $t('vault.itemDetails.moveToTrash')
              "
    ...
    <template #content>
      <div class="p-8 bg-zinc-950 ring-1 ring-white/10 rounded-[32px]">
        <div class="flex items-center gap-4 mb-6">
          ...
          >
            <UIcon
              :name="isTrashed ? 'i-heroicons-exclamation-triangle' : 'i-heroicons-trash'"
              class="size-6"
              :class="isTrashed ? 'text-red-500' : 'text-amber-500'"
            />
          </div>
          <div>
            <h3 class="text-xl font-light text-white">
              {{
                isTrashed
                  ? $t("vault.itemDetails.deletePermanentQuestion")
                  : $t("vault.itemDetails.moveToTrashQuestion")
              }}
            </h3>
            <p class="text-sm text-zinc-500">
              {{
                isTrashed
                  ? $t("vault.itemDetails.deletePermanentWarning")
                  : $t("vault.itemDetails.moveToTrashWarning")
              }}
            </p>
          </div>
        </div>
        ...
        <div class="flex gap-4">
          <UButton
            :label="$t('vault.itemDetails.cancel')"
            color="neutral"
            variant="ghost"
            block
            class="flex-1"
