...
const navigation = computed(() => [
    {
        label: t("vault.sidebar.allItems").toUpperCase(),
        links: [
            {
    ...
    {
        label: t("vault.sidebar.categories").toUpperCase(),
        links: [
            {
                label: t("vault.sidebar.passwords"),
                icon: "i-heroicons-key",
                key: "passwords",
            },
            {
                label: t("vault.sidebar.cards"),
                icon: "i-heroicons-credit-card",
                key: "cards",
            },
            {
                label: t("vault.sidebar.notes"),
                icon: "i-heroicons-document-text",
                key: "notes",
            },
            {
                label: t("vault.sidebar.documents"),
                icon: "i-heroicons-document",
                key: "documents",
            },
            {
                label: t("vault.sidebar.identities"),
    ...
    {
        label: t("vault.sidebar.folders").toUpperCase(),
...
const selectedLabel = computed(() => {
    ...
    return t("vault.sidebar.allItems");
});
...
const filteredItems = computed(() => {
    ...
    if (s === "trash") {
        ...
        if (s === "passwords")
            result = result.filter((i) => i.item_type === "password");
        else if (s === "cards")
            result = result.filter((i) => i.item_type === "card");
        else if (s === "notes")
            result = result.filter((i) => i.item_type === "note");
        else if (s === "identities")
            result = result.filter((i) => i.item_type === "identity");
        else if (s === "documents")
            result = result.filter((i) => i.item_type === "document");
    ...
    if (searchQuery.value) {
        ...
        result = result.filter(
            (i) =>
                ...
                (i.item_type === "password" &&
                    (i as any).username?.toLowerCase().includes(q)) ||
                (i.item_type === "identity" &&
                    ((i as any).firstName?.toLowerCase().includes(q) ||
                        (i as any).lastName?.toLowerCase().includes(q))) ||
                (i.item_type === "card" &&
                    (i as any).holder?.toLowerCase().includes(q)) ||
                (i.item_type === "document" &&
                    (i as any).fileName?.toLowerCase().includes(q)),
        );
    }
...
const passwordCount = computed(
    () => items.value.filter((i) => i.item_type === "password").length,
);
const cardCount = computed(
    () => items.value.filter((i) => i.item_type === "card").length,
);
const noteCount = computed(
    () => items.value.filter((i) => i.item_type === "note").length,
);
const identityCount = computed(
    () => items.value.filter((i) => i.item_type === "identity").length,
);
const documentCount = computed(
    () => items.value.filter((i) => i.item_type === "document").length,
);
...
const securityHealth = computed(() => {
    const scoredItems = items.value.filter(
        (i) =>
            !i.trashed &&
            i.item_type === "password" &&
            i.security_score != null,
    );
    if (scoredItems.length === 0) return 100;

    const totalScore = scoredItems.reduce(
        (acc, curr) => acc + (curr.security_score || 0),
        0,
    );
...
function getSubtitle(item: VaultItem) {
    if (item.item_type === "password")
        return (item as any).username || t("vault.types.password.label");
    if (item.item_type === "card")
        return (item as any).number
            ? `•••• ${(item as any).number.slice(-4)}`
            : t("vault.types.card.label");
    if (item.item_type === "identity")
        return (item as any).email || t("vault.types.identity.label");
    if (item.item_type === "note") return t("vault.types.note.label");
    if (item.item_type === "document")
        return (item as any).fileName || t("vault.sidebar.documents");
    return "...";
}
...
const toggleLabel = computed(() => {
    ...
    return open.value ? t("vault.sidebar.hide") : t("vault.sidebar.show");
});
...
const menuItems = computed<NavigationMenuItem[][]>(() => [
    [
        ...
        },
        {
            label: t("vault.sidebar.trash"),
            icon: "i-heroicons-trash",
            active: selected.value === "trash",
            onSelect: () => {
                selected.value = "trash";
            },
        },
    ],
    [
        {
            label: t("vault.sidebar.passwords"),
            icon: "i-heroicons-key",
            active: selected.value === "passwords",
            onSelect: () => {
                selected.value = "passwords";
            },
        },
        {
            label: t("vault.sidebar.cards"),
            icon: "i-heroicons-credit-card",
            active: selected.value === "cards",
            onSelect: () => {
        ...
        },
        {
            label: t("vault.sidebar.documents"),
            icon: "i-heroicons-document",
            active: selected.value === "documents",
            onSelect: () => {
                selected.value = "documents";
            },
        },
        {
            label: t("vault.sidebar.identities"),
            icon: "i-heroicons-user-circle",
            active: selected.value === "identities",
            onSelect: () => {
                selected.value = "identities";
            },
        },
    ],
    folders.value.map((f) => ({
...
const handleCreateFolder = async () => {
    ...
    try {
        ...
        newFolderName.value = "";
        toast.add({
            title: t("vault.folderModal.newFolder"),
            color: "success",
        });
    } catch (e) {
        toast.add({ title: t("vault.itemDetails.error"), color: "error" });
    } finally {
...
const handleUpdateFolder = async () => {
    ...
    try {
        ...
        toast.add({ title: t("vault.itemDetails.updated"), color: "success" });
    } catch (e) {
        toast.add({ title: t("vault.itemDetails.error"), color: "error" });
    } finally {
        isFolderActionLoading.value = false;
...
const handleDeleteFolder = async (id: string) => {
    if (confirm(t("vault.folderModal.deleteDesc"))) {
        isFolderActionLoading.value = true;
        try {
            ...
            toast.add({
                title: t("vault.folderModal.delete"),
                color: "success",
            });
        } catch (e) {
            toast.add({ title: t("vault.itemDetails.error"), color: "error" });
...
<template>
    <div
        class="min-h-screen vault-bg text-white font-sans selection:bg-primary-500/30"
    >
        <UDashboardGroup unit="rem">
            ...
            >
                <template #header="{ collapsed: isCollapsed, collapse }">
                    ...
                    >
                        <div
                            v-if="!isCollapsed"
                            class="flex items-center gap-3 min-w-0 pr-2"
                        >
                            <div
                            ...
                            <div class="flex flex-col">
                                ...
                                >
                                <span
                                    class="text-[10px] text-gray-500 uppercase tracking-wider font-medium mt-1"
                                    >{{
                                        $t("vault.dashboard.securityStatus")
                                    }}</span
                                >
                            </div>
                        </div>
                ...
                <div v-if="!collapsed" class="px-4 pt-2">
                    <UButton
                        ...
                        :label="$t('vault.sidebar.manageFolders')"
                        color="neutral"
                        variant="ghost"
                        block
                ...
                <template #footer="{ collapsed: isCollapsed }">
                    <div class="border-t border-white/5">
                        ...
                        <div v-if="!isCollapsed" class="px-3 pt-4 pb-2">
                            <NuxtLink
                                to="/boveda/perfil"
                                class="flex items-center gap-3 p-2 text-gray-500 hover:text-gray-300 transition-colors mb-2"
                            >
                                <UIcon
                            ...
                            <div
                                class="flex items-center gap-3 p-2 rounded-xl bg-black border border-white/5 transition-colors group"
                            >
                                <div
                                ...
                                <div class="min-w-0 flex-1">
                                    <div
                                        class="text-xs truncate font-bold text-white tracking-tight leading-none mb-1"
                                    >
                                        {{ displayName }}
                                    </div>
                                    <div
                                        class="text-[9px] text-zinc-600 font-bold uppercase tracking-wider"
                                    >
                                        Admin Session
                                    </div>
                                </div>
                        ...
                        <div
                            v-else
                            class="flex flex-col items-center gap-4 pt-4 pb-2"
                        >
                            <UIcon
                                name="i-heroicons-cog-6-tooth"
                                class="size-4 text-gray-500 hover:text-gray-300"
                            />
                            <div
            ...
            <UDashboardPanel>
                ...
                <div
                    class="px-4 md:px-8 pt-8 pb-4 overflow-y-auto min-h-full bg-black"
                ...
                >
                        ...
                        >
                            ...
                            >
                                ...
                                >
                                    ...
                                    >
                                        <span
                                            class="text-[9px] font-bold text-zinc-600 uppercase tracking-[0.3em]"
                                            >{{
                                                $t(
                                                    "vault.dashboard.securityStatus",
                                                )
                                            }}</span
                                        >
                                        <UIcon
                                            name="i-heroicons-shield-check"
                                            class="size-4 text-[#10b981]"
                                        />
                                    </div>

                                    <div class="flex items-end gap-4">
                                        <span
                                            class="text-6xl font-black tracking-tighter text-white"
                                            >{{ securityHealth }}%</span
                                        >
                                        <div class="mb-2">
                                            <div
                                                class="px-2 py-0.5 bg-[#10b981]/10 border border-[#10b981]/30 rounded text-[9px] font-bold text-[#10b981] uppercase tracking-wider"
                                            >
                                                {{
                                                    $t(
                                                        "vault.dashboard.optimized",
                                                    )
                                                }}
                                            </div>
                                        </div>
                                    ...
                                    <p
                                        class="text-[11px] text-zinc-600 font-medium leading-tight max-w-sm"
                                    >
                                        {{
                                            $t("vault.dashboard.optimizedDesc")
                                        }}
                                    </p>
                                </div>
                            </div>
                            ...
                            <!-- Quick Stats Grid -->
                            <div
                                class="flex-1 grid grid-cols-2 lg:grid-cols-4 gap-3"
                            >
                                <div
                                    v-for="stat in [
                                        {
                                ...
                                >
                                    <UIcon
                                        :name="stat.icon"
                                        class="size-4 text-zinc-700 transition-colors group-hover:text-[#10b981] mb-3"
                                    />
                                    <div>
                                        <div
                                            class="text-2xl font-bold text-white mb-0.5"
                                        >
                                            {{ stat.count }}
                                        </div>
                                        <div
                                            class="text-[9px] font-bold text-zinc-700 uppercase tracking-widest group-hover:text-zinc-500 transition-colors truncate"
                                        >
                                            {{ stat.label }}
                                        </div>
                                    </div>
                                ...
                                >
                                    <div class="flex items-center gap-2">
                                        ...
                                        />
                                        <span
                                            class="text-[9px] font-bold text-zinc-700 uppercase tracking-[0.2em] group-hover:text-zinc-400 transition-colors"
                                            >{{
                    ...
                    >
                        ...
                        <UInput
                            ...
                            variant="none"
                            :placeholder="$t('vault.dashboard.search')"
                            class="w-full sm:w-80"
                            :ui="{
                                base: 'bg-transparent text-xs font-bold tracking-widest text-zinc-400 focus:text-white transition-colors uppercase',
                            }"
                            autocomplete="off"
                        />
                        ...
                        <UButton
                            ...
                            :ui="{
                                base: 'text-[#10b981] font-black uppercase tracking-[0.2em] text-[10px]',
                            }"
                        >
                            {{ $t("vault.dashboard.newItem") }}
                        </UButton>
                    </div>
                    ...
                    <div class="card-grid relative">
                        ...
                        >
                            ...
                            >
                                ...
                                <div class="mb-4">
                                    <h3
                                        class="text-base font-bold text-white mb-2 tracking-tight group-hover:text-[#10b981] transition-colors leading-none truncate"
                                    >
                                        {{ i.title }}
                                    </h3>
                                    <div
        ...
        <UModal v-model:open="isFoldersModalOpen">
            <template #content>
                <div
                    class="p-6 bg-[#050505] border border-white/10 rounded-2xl shadow-2xl"
                >
                    <div class="flex items-center justify-between mb-6">
                        ...
                        >
                            ...
                            />
                            {{ $t("vault.folderModal.title") }}
                        </h3>
                        <UButton
                            icon="i-heroicons-x-mark"
                            color="neutral"
                            variant="ghost"
                            @click="isFoldersModalOpen = false"
                        />
                    </div>
                    ...
                    <div class="flex items-end gap-2 mb-8">
                        <UFormField
                            :label="$t('vault.folderModal.newFolder')"
                            class="flex-1"
                            :ui="{ label: 'text-xs text-gray-500' }"
                        >
                            <UInput
                                v-model="newFolderName"
                                :placeholder="
                                    $t('vault.folderModal.namePlaceholder')
                                "
                                @keyup.enter="handleCreateFolder"
                                variant="none"
                                class="bg-white/5 rounded-lg border border-white/10 focus-within:border-white/20"
                            />
                        </UFormField>
                        <UButton
                            icon="i-heroicons-plus"
                            ...
                            <template v-else>
                                <div class="flex items-center gap-3">
                                    ...
                                    <span
                                        ...
                                        class="text-[9px] uppercase tracking-widest text-primary-400 font-bold border border-primary-400/20 px-1.5 py-0.5 rounded"
                                        >Default</span
                                    >
                                </div>
