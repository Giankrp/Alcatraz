<script setup lang="ts">
  ...
  const { items, searchQuery, fetchItems, folders, addFolder, updateFolder, deleteFolder } =
    useVault()
  ...
  const navigation = computed(() => [
    {
      ...
      links: [
        ...
        { label: t("vault.sidebar.trash"), icon: "i-heroicons-trash", key: "trash" },
      ],
    },
    {
      label: t("vault.sidebar.categories").toUpperCase(),
      links: [
        { label: t("vault.sidebar.passwords"), icon: "i-heroicons-key", key: "passwords" },
        {
  ...
  const securityHealth = computed(() => {
    const scoredItems = items.value.filter(
      i => !i.trashed && i.item_type === "password" && i.security_score != null,
    )
  ...
  function getSubtitle(item: VaultItem) {
    ...
    if (item.item_type === "identity") return (item as any).email || t("vault.types.identity.label")
    if (item.item_type === "note") return t("vault.types.note.label")
    if (item.item_type === "document") return (item as any).fileName || t("vault.sidebar.documents")
    return "..."
  }
  ...
  const toggleLabel = computed(() => {
    if (isDesktop.value)
      return collapsed.value ? t("vault.sidebar.expand") : t("vault.sidebar.collapse")
  ...
  const menuItems = computed<NavigationMenuItem[][]>(() => [
    ...
    [
      {
        label: t("vault.sidebar.identity"),
  ...
  const handleCreateFolder = async () => {
    ...
    try {
      ...
      newFolderName.value = ""
      toast.add({ title: t("vault.folderModal.newFolder"), color: "success" })
    } catch (e) {
  ...
  const handleDeleteFolder = async (id: string) => {
    if (confirm(t("vault.folderModal.deleteDesc"))) {
      ...
      try {
        ...
        toast.add({ title: t("vault.folderModal.delete"), color: "success" })
      } catch (e) {
        toast.add({ title: t("vault.itemDetails.error"), color: "error" })
      } finally {
        isFolderActionLoading.value = false
          ...
          >
            <div v-if="!isCollapsed" class="flex items-center gap-3 min-w-0 pr-2">
              ...
              <div class="flex flex-col">
                ...
                <span class="text-[10px] text-gray-500 uppercase tracking-wider font-medium mt-1">{{
            ...
            <UButton
              ...
              :aria-label="isCollapsed ? $t('vault.sidebar.expand') : $t('vault.sidebar.collapse')"
              @click="collapse?.(!isCollapsed)"
            >
              <span class="sr-only">{{
                isCollapsed ? $t("vault.sidebar.expand") : $t("vault.sidebar.collapse")
              }}</span>
              <UIcon
                :name="
                  isCollapsed
          ...
          <div class="border-t border-white/5">
            ...
            <div v-if="!isCollapsed" class="px-3 pt-4 pb-2">
              ...
              >
                <div
                  ...
                  :style="{
                    background: avatarUrl ? 'black' : avatarColor,
                  }"
                >
                ...
                <div class="min-w-0 flex-1">
                  ...
                  <div class="text-[9px] text-zinc-600 font-bold uppercase tracking-wider">
                    Admin Session
                  </div>
                </div>
              </div>
            ...
            <div class="px-3 pb-4 pt-1">
              <UButton
                ...
                :label="isCollapsed ? undefined : $t('vault.sidebar.logout')"
                color="neutral"
                variant="ghost"
                block
                :class="[
              ...
              >
                <div class="relative z-10 flex flex-col h-full justify-between gap-4">
                  <div class="flex items-center justify-between">
                    <span class="text-[9px] font-bold text-zinc-600 uppercase tracking-[0.3em]">{{
                      $t("vault.dashboard.securityStatus")
                    }}</span>
          ...
          <div class="flex items-end gap-2 mb-8">
            ...
            >
              <UInput
                ...
                :placeholder="$t('vault.folderModal.namePlaceholder')"
