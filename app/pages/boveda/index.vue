    ...
    meta: [
        {
            name: "description",
            content:
                "Gestor de contraseñas y notas cifradas — Bóveda de Alcatraz.",
        },
    ],
});
...
function getSubtitle(item: VaultItem) {
    if (item.item_type === "password")
        return (item as any).username || "Usuario cifrado";
    if (item.item_type === "card")
        return (item as any).number
            ? `•••• ${(item as any).number.slice(-4)}`
            : "Tarjeta cifrada";
    if (item.item_type === "identity")
        return (item as any).email || "Identidad cifrada";
    if (item.item_type === "note") return "Nota segura";
    if (item.item_type === "document")
        return (item as any).fileName || "Documento cifrado";
    return "...";
}
...
const handleDeleteFolder = async (id: string) => {
    if (
        confirm(
            "¿Estás seguro de que quieres eliminar esta carpeta? Los elementos se moverán a Personal.",
        )
    ) {
        isFolderActionLoading.value = true;
        try {
                    ...
                    <div class="border-t border-white/5">
                        ...
                        <div class="px-3 pb-4 pt-1">
                            <UButton
                                ...
                                :label="
                                    isCollapsed ? undefined : 'Cerrar sesión'
                                "
                                color="neutral"
                                variant="ghost"
                                block
                        ...
                        >
                            ...
                            >
                                ...
                                >
                                    ...
                                    >
                                        Protocolos de cifrado activos y
                                        validados.
                                    </p>
                                </div>
                            </div>
