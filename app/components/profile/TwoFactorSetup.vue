...
<template>
  <UModal v-model:open="isOpen" :prevent-close="step === 3">
    <template #content>
      <div
        class="p-8 bg-black border border-emerald-500/20 rounded-2xl max-w-md mx-auto shadow-2xl overflow-hidden relative"
      >
        <!-- Decoration -->
        <div
          class="absolute -top-24 -right-24 size-48 bg-emerald-500/5 blur-[60px] rounded-full pointer-events-none"
        />

        <!-- STEP 1: Introduction -->
        <div v-if="step === 1" class="text-center animate-fade-in">
          <div
            class="size-16 bg-emerald-500/10 border border-emerald-500/20 rounded-2xl flex items-center justify-center mx-auto mb-6"
          >
        ...
        <div v-if="step === 2" class="animate-fade-in">
          <h3 class="text-lg font-bold text-white uppercase tracking-widest mb-6 text-center">
            {{ $t("auth.setup.scan") }}
          </h3>

          <div
            class="bg-white p-4 rounded-xl mx-auto mb-8 w-fit shadow-[0_0_30px_rgba(255,255,255,0.1)]"
          >
          ...
          <div class="space-y-4 mb-8">
            <p class="text-[10px] text-zinc-500 uppercase tracking-widest text-center">
              {{ $t("auth.setup.manual") }}
              <code class="text-emerald-400 font-mono">{{ secret }}</code>
            </p>
        ...
        <div v-if="step === 3" class="animate-fade-in">
          ...
          <div
            class="grid grid-cols-2 gap-3 mb-8 bg-zinc-950 p-6 rounded-xl border border-zinc-900 font-mono text-zinc-300 relative overflow-hidden"
          >
            <div
              v-for="code in backupCodes"
              :key="code"
              class="text-center py-1 text-sm tracking-wider"
            >
              {{ code }}
            </div>
            <!-- Grain effect -->
            <div
              class="absolute inset-0 opacity-20 pointer-events-none bg-[url('https://grainy-gradients.vercel.app/noise.svg')]"
            />
          </div>
