<script setup lang="ts">
  ...
  const schema = z.object({
    title: z.string().min(1, t("vault.forms.passwordForm.zodRequired")),
    username: z.email(t("vault.forms.passwordForm.zodEmail")).or(z.string()),
    password: z.string(),
    url: z.url(t("vault.forms.passwordForm.zodUrl")).optional().or(z.literal("")),
    folder: z.string(),
  })
...
<template>
  <FormLayout
    :title="initialData ? $t('vault.forms.passwordForm.edit') : $t('vault.forms.passwordForm.new')"
    :can-save="true"
  ...
  >
    <UForm :schema="schema" :state="state" class="space-y-6" @submit="onSubmit">
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <UFormField :label="$t('vault.forms.passwordForm.titleLabel')" name="title">
          <UInput
            v-model="state.title"
            :placeholder="$t('vault.forms.passwordForm.titlePlaceholder')"
        ...
        <UFormField :label="$t('vault.forms.passwordForm.folderLabel')" name="folder">
          <USelect
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <UFormField :label="$t('vault.forms.passwordForm.usernameLabel')" name="username">
          <UInput
            v-model="state.username"
            :placeholder="$t('vault.forms.passwordForm.usernamePlaceholder')"
        ...
        <UFormField :label="$t('vault.forms.passwordForm.passwordLabel')" name="password">
          <div class="flex items-center gap-2 w-full">
            ...
            <UPopover
              ...
              :ui="{ content: 'bg-transparent dark:bg-transparent shadow-none ring-0' }"
            >
              <UButton
              ...
              <template #content>
                ...
                >
                  <div class="flex items-center justify-between border-b border-white/10 pb-4">
                    <span
                      class="font-mono text-green-400 break-all text-sm font-semibold select-all"
                    ...
                    <UButton
                      ...
                      type="button"
                      @click="doGenerate()"
                      class="shrink-0 ml-3 hover:bg-white/5 opacity-70 hover:opacity-100"
                    />
                  </div>
                  ...
                  <div class="space-y-5 pt-1">
                    <div class="space-y-3">
                      ...
                      >
                        <span>{{ $t("vault.forms.passwordForm.lengthLabel") }}</span>
                    ...
                    <div class="space-y-3">
                      <div class="flex justify-between items-center">
                        <span class="text-xs text-neutral-300 font-medium">{{
                          $t("vault.forms.passwordForm.uppercaseLabel")
                        }}</span>
                        <USwitch v-model="includeUppercase" />
                      </div>
                      <div class="flex justify-between items-center">
                        <span class="text-xs text-neutral-300 font-medium">{{
                          $t("vault.forms.passwordForm.numbersLabel")
                        }}</span>
                        <USwitch v-model="includeNumbers" />
                      </div>
                      <div class="flex justify-between items-center">
                        <span class="text-xs text-neutral-300 font-medium">{{
                          $t("vault.forms.passwordForm.symbolsLabel")
                        }}</span>
                  ...
                  <UButton
                    block
                    class="btn-accent border border-green-500/30 font-semibold mt-2"
                    size="md"
                    type="button"
                    @click="applyGeneratedPassword"
                  >
                    {{ $t("vault.forms.passwordForm.useBtn") }}
                  </UButton>
                </div>
              </template>
            </UPopover>
      ...
      <UFormField :label="$t('vault.forms.passwordForm.urlLabel')" name="url">
        <UInput
          v-model="state.url"
          :placeholder="$t('vault.forms.passwordForm.urlPlaceholder')"
      ...
      <div class="rounded-xl bg-white/5 p-4 border border-white/5 flex gap-4 items-start pt-6">
        ...
        <div>
          <h4 class="text-sm font-medium text-white mb-1">
            {{ $t("vault.forms.passwordForm.noticeTitle") }}
          </h4>
          <p class="text-xs text-gray-400 leading-relaxed">
            {{ $t("vault.forms.passwordForm.noticeDesc") }}
          </p>
        </div>
      </div>
    </UForm>
